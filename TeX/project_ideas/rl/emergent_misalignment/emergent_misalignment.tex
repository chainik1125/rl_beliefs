...
Let $\mathcal{X}$ be a finite alphabet and let $\{T(x)\}_{x\in\mathcal{X}}$ be the transfer matrices of a (finite-state) generalized hidden Markov model (GHMM). We follow the convention that
\begin{equation}
  T(x)_{s,s'} \;=\; Q(s',x\mid s),
\end{equation}
so that $T(x)$ is entrywise nonnegative and the \emph{net} transition operator
\begin{equation}
  T \;\coloneqq\; \sum_{x\in\mathcal{X}} T(x)
\end{equation}
is row-stochastic, i.e.\ $T\mathbf{1}=\mathbf{1}$ where $\mathbf{1}$ is the all-ones column vector. The stationary distribution is the left eigenvector $\pi^\top T=\pi^\top$ normalized by $\pi^\top\mathbf{1}=1$. The probability of a length-$\ell$ sequence $x_{1:\ell}$ under the GHMM is
\begin{equation}
  Q_M(X_{1:\ell}=x_{1:\ell})
  \;=\;
  \pi^\top\, T(x_1)\cdots T(x_\ell)\,\mathbf{1}.
\end{equation}
Hidden Markov models (HMMs) are the special case in which $T(x)_{s,s'}=Q(s',x\mid s)$ arises from a standard hidden-state chain with emissions.

\paragraph{Tensor-product (factored) processes.}
...
\end{equation}
A simple Z1R-like choice that preserves this split, while retaining the emission probabilities of the usual Z1R (i.e.\ $Q(0\mid S0)=1$, $Q(1\mid S1)=1$, and $Q(0\mid SR)=Q(1\mid SR)=\tfrac12$), is
\begin{equation}
  T_0' \;=\;
  \begin{pmatrix}
    1 & 0 & 0\\
